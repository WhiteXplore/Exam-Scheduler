  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddSubjectOpen"
      >
        <icon name="add-students"></icon>Add Subject
      </button>
    </div>
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-3 border-b w-[15%]">
                  {{ item.entry_subject_year }}
                </td>
                <td class="px-4 py-3 border-b w-[15%]">
                  {{ item.entry_subject_code }}
                </td>
    ...
    <div v-if="isAddSubjectOpen"><addSubject /></div>
  </div>
</template>

<script>
import axios from "axios";
import addSubject from "../actions/add-subject.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "SubjectTable",
  components: { addSubject, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
