  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddDayOpen"
      >
        <icon name="add-students"></icon>Add Day
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Day</th>
    ...
    <div v-if="isAddDayOpen"><addDay /></div>
    <div v-if="isDeleteDayOpen">
...
import addDay from "../actions/add-day.vue";
...
export default {
  name: "ClassTable",
  components: { addDay, deleteEntryClass, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleAddDayOpen() {
      this.isAddDayOpen = !this.isAddDayOpen;
    },
    toggleDeleteClassOpen(classId) {
      this.selectedClass = classId;
      this.isDeleteDayOpen = !this.isDeleteDayOpen;
    },
    async fetchClassData() {
      try {
        const response = await axios.get("http://localhost:8000/entry/get-day");
