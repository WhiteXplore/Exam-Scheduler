  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddSetOpen"
      >
        <icon name="add-students"></icon>Add Set
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Set</th>
    ...
    <div v-if="isAddSetOpen"><addSet /></div>
  </div>
</template>

<script>
import axios from "axios";
import addSet from "../actions/add-set.vue";
import icon from "@/assets/icons/icon.vue";
import { entryStore } from "@/PiniaStore/entry-store";

export default {
  name: "ClassTable",
  components: { addSet, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleAddSetOpen() {
      this.isAddSetOpen = !this.isAddSetOpen;
    },
    ...
    async fetchClassData() {
      try {
        const response = await axios.get("http://localhost:8000/entry/get-set");
