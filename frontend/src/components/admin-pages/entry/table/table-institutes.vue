  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddInstitutesOpen"
      >
        <icon name="add-students"></icon>Add Institutes
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Institutes</th>
    ...
    <div v-if="isAddInstitutesOpen"><addInstitutes /></div>
  </div>
</template>

<script>
import axios from "axios";
import addInstitutes from "../actions/add-institutes.vue";
import icon from "@/assets/icons/icon.vue";
import { entryStore } from "@/PiniaStore/entry-store";

export default {
  name: "ClassTable",
  components: { addInstitutes, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
      currentPage: 1,
      isAddInstitutesOpen: false,
  ...
  methods: {
    ...
    toggleAddInstitutesOpen() {
      this.isAddInstitutesOpen = !this.isAddInstitutesOpen;
    },
    ...
    async fetchClassData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/entry/get-institutes"
        );
