  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddClassOpen"
      >
        <icon name="add-students"></icon>Add Class
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Class</th>
    ...
    <div v-if="isAddClassOpen"><addEntryClass /></div>
    <div v-if="isDeleteClassOpen">
      <deleteEntryClass :classId="selectedClass" />
    </div>
  </div>
</template>
...
import addEntryClass from "../actions/add-EntryClass.vue";
import deleteEntryClass from "../actions/delete-EntryClass.vue";
import icon from "@/assets/icons/icon.vue";
import { entryStore } from "@/PiniaStore/entry-store";

export default {
  name: "ClassTable",
  components: { addEntryClass, deleteEntryClass, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleDeleteClassOpen(classId) {
      ...
      this.isDeleteClassOpen = !this.isDeleteClassOpen;
    },
    async fetchClassData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/entry/get-class"
        );
