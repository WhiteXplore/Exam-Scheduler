  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddDesignationOpen"
      >
        <icon name="add-students"></icon>Add Designation
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Designation</th>
    ...
    <div v-if="isAddDesignationOpen"><addDesignation /></div>
  </div>
</template>

<script>
import axios from "axios";
import addDesignation from "../actions/add-designation.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "RoomTable",
  components: { addDesignation, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleAddDesignationOpen() {
      this.isAddDesignationOpen = !this.isAddDesignationOpen;
    },
    async fetchDesignationData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/maintenance/get-designation"
        );
