  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddSchoolYearOpen"
      >
        <icon name="add-students"></icon>Add School Year
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">School Year</th>
    ...
    <div v-if="isAddClassOpen"><addSchoolYear /></div>
  </div>
</template>

<script>
import axios from "axios";
import addSchoolYear from "../actions/add-school-year.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "ClassTable",
  components: { addSchoolYear, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleAddSchoolYearOpen() {
      this.isAddClassOpen = !this.isAddClassOpen;
    },
    async fetchClassData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/maintenance/get-school-year"
        );
