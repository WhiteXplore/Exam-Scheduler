  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddDepartmentOpen"
      >
        <icon name="add-students"></icon>Add Department
      </button>
    </div>
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-3 border-b w-[15%]">
                  {{ item.maintenance_department_code }}
                </td>
                <td class="px-4 py-3 border-b w-[20%]">
                  {{ item.maintenance_department }}
                </td>
    ...
    <div v-if="isAddDepartmentOpen"><addDepartment /></div>
  </div>
</template>

<script>
import axios from "axios";
import addDepartment from "../actions/add-department.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "SubjectTable",
  components: { addDepartment, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  computed: {
    ...
    filteredData() {
      ...
      return this.items.filter((item) => {
        const departmentCode = item.maintenance_department_code
          ? item.maintenance_department_code.toLowerCase()
          : "";
        const departmentName = item.maintenance_department
          ? item.maintenance_department.toLowerCase()
          : "";

        return departmentCode.includes(query) || departmentName.includes(query);
      });
    },
    paginatedData() {
  ...
  methods: {
    ...
    toggleAddDepartmentOpen() {
      this.isAddDepartmentOpen = !this.isAddDepartmentOpen;
    },
    async fetchDepartmentData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/maintenance/get-department"
        );
