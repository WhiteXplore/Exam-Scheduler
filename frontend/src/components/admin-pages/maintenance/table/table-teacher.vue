<template>
  <div>
    <div class="text-sm flex justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button
          ...
          @click="toggleAddTeacherOpen"
        >
          <icon name="add-students"></icon>Add Teacher
        </button>
      </div>
    </div>
          ...
          <table class="w-full border-collapse text-text text-[13px]">
            <thead>
              <tr class="bg-[#147452] text-white">
                ...
                <th class="px-4 py-2 text-left border-b w-[15%]">Rank</th>
                <th class="px-4 py-2 text-left border-b w-[20%]">Department</th>
                <th class="px-4 py-2 text-left border-b w-[20%]">
                  Designation
                </th>
                <th class="px-4 py-2 text-left border-b w-[15%]">Status</th>
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-3 border-b w-[20%] whitespace-nowrap">
                  {{ item.entry_first_name }} {{ item.entry_middle_name }}
                  {{ item.entry_last_name }}, {{ item.entry_salutation }}
                </td>
                <td class="px-4 py-3 border-b w-[15%]">
                  {{ item.entry_rank }}
                </td>
                <td class="px-4 py-3 border-b w-[20%]">
                  {{ item.entry_department }}
                </td>
                <td class="px-4 py-3 border-b w-[20%]">
                  {{ item.entry_designation }}
                </td>
                <td class="px-4 py-3 border-b w-[15%]">
                  {{ item.entry_status }}
                </td>
    ...
    <div v-if="isAddTeacherOpen"><addTeacher /></div>
    <div v-if="isAddUploadTeacher"><uploadTeacher /></div>
  </div>
</template>

<script>
import axios from "axios";
import addTeacher from "../actions/add-teacher.vue";
import uploadTeacher from "../actions/upload-teacher.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "TeacherTable",
  components: { addTeacher, icon, uploadTeacher },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  computed: {
    ...
    filteredData() {
      ...
      return this.items.filter((item) =>
        [
          item.entry_first_name,
          item.entry_middle_name,
          item.entry_last_name,
          item.entry_salutation,
          item.entry_rank,
          item.entry_department,
          item.entry_designation,
          item.entry_status,
  ...
  methods: {
    ...
    toggleAddTeacherOpen() {
      this.isAddTeacherOpen = !this.isAddTeacherOpen;
    },
    toggleAddUploadOpen() {
      this.isAddUploadTeacher = !this.isAddUploadTeacher;
    },
    async fetchTeacherData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/entry/get-teacher"
        );
