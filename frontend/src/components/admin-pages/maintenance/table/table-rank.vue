  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddRankOpen"
      >
        <icon name="add-students"></icon>Add Rank
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Rank</th>
    ...
    <div v-if="isAddRankOpen"><addRank /></div>
  </div>
</template>

<script>
import axios from "axios";
import addRank from "../actions/add-rank.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "RoomTable",
  components: { addRank, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
  ...
  methods: {
    ...
    toggleAddRankOpen() {
      this.isAddRankOpen = !this.isAddRankOpen;
    },
    async fetchRankData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/maintenance/get-rank"
        );
