  ...
  <div>
    <div class="text-sm flex justify-between">
      ...
      <button
        ...
        @click="toggleAddSalutationOpen"
      >
        <icon name="add-students"></icon>Add Salutation
      </button>
    </div>
          ...
          >
            <thead>
              <tr class="bg-[#147452] text-white">
                <th class="px-4 py-3 text-left border-b w-2/3">Salutation</th>
    ...
    <div v-if="isAddSalutationOpen"><addSalutation /></div>
  </div>
</template>

<script>
import axios from "axios";
import addSalutation from "../actions/add-salutation.vue";
import icon from "@/assets/icons/icon.vue";

export default {
  name: "RoomTable",
  components: { addSalutation, icon },
  data() {
    return {
      searchQuery: "",
      itemsPerPage: 10,
      currentPage: 1,
      isAddSalutationOpen: false,
      items: [],
    };
  },
  computed: {
  ...
  methods: {
    ...
    toggleAddSalutationOpen() {
      this.isAddSalutationOpen = !this.isAddSalutationOpen;
    },
    async fetchSalutationData() {
      try {
        const response = await axios.get(
          "http://localhost:8000/maintenance/get-salutation"
        );
