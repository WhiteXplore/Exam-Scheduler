<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 bg-black bg-opacity-25 flex justify-center items-center z-50 w-screen"
  >
    <div class="rounded-xl shadow-lg fixed top-40">
      <div class="w-auto bg-white text-text1 text-[13px] rounded-md">
        ...
        >
          <h1 class="font-bold tracking-wide text-lg">Add School Year</h1>
        ...
        <div class="p-5 text-gray-700">
          <div class="flex flex-col space-y-2">
            ...
            <input
              v-model="form.maintenance_school_year"
              type="text"
    ...
    async submitData() {
      try {
        const response = await axios.post(
          "http://localhost:8000/maintenance/add-school-year",
          this.form
        );
